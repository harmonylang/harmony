...
static bool onestep(
    ...
    struct step_output *so = walloc(w, sizeof(struct step_output) + step->nlog * sizeof(hvalue_t), false, false);
    so->after = after;
...
#ifdef STATE_EXTRACT
    extract = dict_new("extract", sizeof(struct step_output), 0, global->nworkers, false);
#endif
