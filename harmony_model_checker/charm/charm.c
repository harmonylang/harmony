        ...
        if (w->index == 0 % w->nworkers) {
            if (done) {
                ...
                printf("    * %u states (time %.2lfs, mem=%.3lfGB)\n", nstates,
                    global.times[PHASE_MODELCHECK_END] - global.times[PHASE_MODELCHECK_START],
                ...
                global.times[PHASE_SCAN_START] = gettime();
...
static void time_info(char *descr, unsigned int start, unsigned int end){
    printf("    * %.2f: %s\n", global.times[end] - global.times[start], descr);
}
...
int exec_model_checker(int argc, char **argv){
    ...
    *global.callstacks = cs;
    ...
    global.times[PHASE_MODELCHECK_START] = gettime();
    worker(&workers[0]);
    ...
    global.times[PHASE_ANALYSIS_START] = gettime();
    ...
    global.times[PHASE_DUMP_START] = gettime();
    ...
    global.times[PHASE_DUMP_END] = gettime();
