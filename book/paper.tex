...
set \{ 0, 1 \}.
By the definition of consensus, we want the following two properties:
\begin{enumerate}
\item \emph{Validity}: a processor can only decide a value that has been proposed;
\item \emph{Agreement}: if two processors decide, they decide the same value.
\end{enumerate}
...
The important properties of this \textbf{select} statement are as follows:
\begin{itemize}
...
\autoref{fig:paxos1} and \autoref{fig:paxos2} contain the code for this Paxos protocol.
A leader first broadcasts a ``p1a'' message (Phase 1, Type A) to the acceptors
containing only the ballot number and then waits for $\texttt{N} - \texttt{F}$ ``p1b''
