...
replicas of the service must coordinate their actions.
One general approach is the
\emph{Replicated State Machine Approach}~\cite{Lam78,S90}.
In this approach, you model your service as a deterministic state
...
\emph{Chain Replication}~\cite{vRS04} is a protocol to do just this.
...
When it does so, it advertises the new update along with the position
in the history.  When the direct successor receives the next update
it is expecting, it appends the update to its history and likely
...
$\mathbf{choose}(\{ \mathit{immortal}, \texttt{True} \})$ can only
evalute to \texttt{False} in case $\textit{immortal}$ is false.
...
it is the head at any time.  Moreover, eventually, when it has
detected all its predecessors having failed, always some replica
...
this message on the network.  The other form is a tuple
