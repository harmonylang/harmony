...
\newglossaryentry{concurrent execution}
{
  ...
  description={is when there are multiple processes executing and their machine
    instructions are interleaved in an unpredictable manner}
}
...
{
  name=invariant,
  description={is a binary property that must hold for every state of a process}
}
...
\newglossaryentry{macro step}
{
  name=macro step,
  description={is a sequence of micro steps with at most one instruction
        externally visible to other processes.  It is the unit of interleaving
        in CXL}
}
\newglossaryentry{micro step}
{
  name=micro step,
  description={is the execution of one machine instruction by a process, updating
        its state and the state of the CXL machine}
}
...
An \emph{invariant}
\index{invariant}
is a property that holds over all reachable states.
