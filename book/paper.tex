...
\newglossaryentry{blocked process}
{
  ...
  description={is a process that cannot change the state or terminate or
      can only do so after another process changes the state first.
      For example, a process that is waiting for a lock to become available}
}
...
\newglossaryentry{machine instruction}
{
  ...
  description={is an atomic operation on the Harmony virtual machine, executed by a process}
}
...
\newglossaryentry{mutual exclusion}
{
  ...
  description={is the property that two processes never enter the same critical section}
}
...
\newglossaryentry{process}
{
  ...
  description={is code in execution.  We do not make the distinction
      between processes and threads.  A process has a current context and
...
\newglossaryentry{producer/consumer problem}
{
  ...
  description={is a synchronization problem whereby one or more producing processes
    submit items and one or more consuming processes want to receive them.  No item
...
middle of a method, and a major source of bugs in practice.
...
are needed, as \texttt{!}\textit{pm}\texttt{.turn} would wrongly evaluate
...
\end{code}
\caption{The \texttt{Lock} interface and implementation in the \texttt{synch} module.}
\label{fig:spinlocks}
\end{figure}
...
\harmonysource{xy}
\end{code}
\caption{Imcomplete code with desired invariant $x + y = 100$.}
\label{fig:xy}
\end{figure}
...
\item $Qenqueue(q, v)$: add $v$ to the tail of the queue;
\item $r = Qdequeue(q)$: returns $r = ()$ if $q$ is empty or $r = (v,)$ (a singleton tuple)
...
\texttt{malloc}() returns a new memory location, which should be released
with \texttt{free}() when it is no longer in use.
The queue maintains a pointer to the first and last elements in the list,
which are \textbf{None} if the queue is empty.

\texttt{Qnew()} returns a new queue object consisting of a \textbf{None} head
and tail pointer and a lock.
\texttt{Qenqueue(q, v)} and \texttt{Qdequeue(q)} both take a pointer $q$ to the
...
separate locks for the head and tail, allowing an enqueue and dequeue operation
to proceed concurrently.  To avoid contention between the head and the tail, the
queue uses a dummy item on the linked list.

\begin{figure}
\begin{code}
\harmonysource{queuetest}
\end{code}
\caption{Test program for a concurrent queue.}
\label{fig:queuetest}
\end{figure}
...
\harmonysource{queue}
\end{code}
\caption{A basic concurrent queue data structure.}
\label{fig:queue}
\end{figure}
...
\end{code}
\caption{A queue with separate locks for enqueuing and dequeuing items.}
\label{fig:queueMS}
\end{figure}
...
for three processes.
\item Optimize your solution to \autoref{ex:xy} to use reader/writer locks.
...
it can be useful for a process to \emph{downgrade} its write lock
...
\texttt{wait} adds the nametag of the process to the bag.
This increments the number of processes in the bag with the same context.
...
memory locations.  There are just two methods:

\vspace{1em}
\begin{tabular}{|l|l|}
\hline
\texttt{malloc()} & allocate and return a pointer to a memory location \\
\hline
...
\texttt{malloc}() is specified to \texttt{None} when running out of
