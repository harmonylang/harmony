...
machine.  The details of this appear in the chapter~\ref{ch:cxlmachine},
but suffice it to
...
\end{verbatim}
\end{code}
However, if the key is an atom, say \texttt{.count}, then you
can write \texttt{d.count} instead of having to write
\texttt{d[.count]} (both will work).
...
is the same as \texttt{(a, b, c)} (which looks like a Python tuple),
which in turn is the same as \texttt{dict\{ 0:a, 1:b, 2:c \}}.
So if \texttt{x == [ False, True ]},
then \texttt{x[0] == False} and \texttt{x[1] == True}.

Dictionaries have two handy unary operators.  If \texttt{d} is a
...
then \texttt{f x}, \texttt{f(x)}, and \texttt{f[x]} are all
