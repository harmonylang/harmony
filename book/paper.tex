...
should equal $q$$\rightarrow$$\mathtt{head}$;
Moreover, $q$$\rightarrow$$\texttt{tail}$$\rightarrow$$next$ should be \texttt{None}.
...
Values can be added using \texttt{insert}.
There is also a read-only operation: \texttt{contains} checks if
a particular value is in the list.
\glsadd{linearizable}%

The read-only operation \texttt{contains} is wait\_free: it scans
...
The \texttt{insert(\textit{lst}, $v$)} method first
...
hand-over-hand.
The \texttt{contains} method can simply scan through the list
...
\end{code}
\caption{\harmonylink{code/linkedlist.hny} Linked list with non-blocking read operation.}
...
\section*{Exercises}
\begin{problems}
\item Add read-only methods to the data structure in \autoref{fig:linkedlist}
