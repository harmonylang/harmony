...
which allow multiple threads in a critical section if they are
...
\chapter{Binary Semaphores and Blocking}
\label{ch:synch}
\index{lock}%
\index{synch module}%
...
thread needs to wait until all workers are done.  Use a semaphore for this:
the \texttt{main} thread should use \texttt{P} to wait for each
...
A critical section allows only one thread to execute its code at a time.
When a thread arrives at a critical section,
...
semaphore for each of the \texttt{N} threads.
Before thread $i$ enters a round, it first sends a signal to every
other thread and then waits until it receives a signal from
every other thread.  Thread $i$ sends a signal to thread $j$
by incrementing \texttt{sema[$j$]} (using \texttt{V}),
while thread $i$ waits for a signal be decrementing \texttt{sema[$i$]}
(using \texttt{P}).
