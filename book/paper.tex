...
\harmonysource{hoare}
\end{code}
\caption{\harmonylink{code/hoare.hny} Implementation of Hoare monitors.}
\label{fig:hoare}
\end{figure}

\begin{figure}
\begin{code}
\harmonysource{hoaretest}
\end{code}
\caption{\harmonylink{code/hoaretest.hny} Example of Hoare monitors.}
\label{fig:hoaretest}
\end{figure}
...
Each condition variable also keeps track of the number of processes waiting
...
\autoref{fig:hoaretest} presents a simple example lifted directly from
the original paper~\cite{Hoare74}.
In the design proposed by Hoare, the monitor methods automatically acquire
and release the monitor mutex.

\texttt{wait} increments the condition's counter, releases the monitor mutex,
blocks while trying to procure the condition's semaphore, and finally decrements
the counter---in much the same way as we have seen for SBS.
\texttt{signal} checks to see if the condition's count is non-zero, and if so
vacates the condition's semaphore---passing the baton to
a process trying to procure it---and then blocks by procuring the mutex again.
