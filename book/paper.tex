...
we have to show how \texttt{Lock}, <{acquire}>, and <{release}>
...
(by calling either <{release}> or \texttt{wait}).
...
by wrapping each \texttt{wait} operation in a <{while}> statement that
...
Note that \texttt{wait} is always invoked within a <{while}>
...
\texttt{wait} within a <{while}> loop, re-checking the condition
...
<{acquire}> and \texttt{wait} should only be used if it is certain that they will not
