...
Here <{s}> contains the context and <{r}> is a Harmony value.
It causes a thread with the context contained in <{s}> to be added to
the state that has just executed the <{stop s}>
...
The Harmony <{save}> expression (\autoref{ap:context}) returns a tuple
...
\hline
Context & (generated by <{stop}> or <{save}> expression) \\
\hline
\end{tabular}
\vspace{1em}
...
returns <{r}> from the <{save e}> expression.  See \autoref{fig:fork}
for an example of how this could be used to \emph{fork} a thread into
two threads.
...
has executed a <{stop}> or <{save}> expression.
The <{stop}> or <{save}> expression returns value <{e}>.
The same context can be started multiple times, allowing threads to
\emph{fork}.  See \autoref{fig:fork} for an example.
