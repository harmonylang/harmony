...
we will ignore that in this book.
...
\item \textit{status}: either \texttt{.idle} or
$(\mathtt{.busy}, \mathit{coordinator}, \mathit{oldbalance})$,
where \textit{coordinator} is the identifier of the coordinator
of a transaction and \textit{oldbalance} is the balance
...
funds, the bank changes its status to \texttt{.busy} and keeps track of
who to coordinator is 
