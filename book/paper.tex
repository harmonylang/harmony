...
\item $r = \mathtt{get}(q)$: returns $r = ()$ if $q$ is empty or $r = (v,)$ (a singleton tuple)
...
\item contexts (\autoref{app:context}) are ordered first by name tag, then by program counter, then by hash.
\end{itemize}
