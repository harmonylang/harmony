...
The purpose here is to check that the code doesn't simply always abort---there
are executions where the code commits.
In practice, hopefully \emph{most} executions commit, but this is not something
the Harmony model checker can check.
...
Also, if Alice is honest, only Alice can have seen the message containing $N_B$.

Thus, the intended security property of this protocol is that if Alice and Bob are both
honest, only Alice and Bob learn $N_A$ and $N_B$ and only if they are trying to communicate
with one another.
...
To demonstate the bug, we need a third party, which we will call Corey.
...
If, at some point, Corey finds the nonce that Bob generates for Alice, then Corey
...
\item Upon receipt, Bob, as usual, creates a new nonce $N_B$ and sends
$\langle 2, N_A, N_B \rangle_\mathtt{Alice}$ to Alice;
...
\item Corey learns $N_B$, which it was not supposed to as Bob is only willing to
talk to Alice and created $N_B$ explicitly for Alice.
\end{enumerate}

Corey can use $N_B$ to send $\langle 3, N_B \rangle_\mathtt{Bob}$ to Bob.
This is identical to the situation in which Alice tried to set up a connection to Bob,
so Bob now thinks it is talking to Alice, even though Alice never tried to
communicate with Bob.  The security property is violated because Corey discovers $N_B$
even though Alice and Bob are both honest and are not trying to communicate with one
another.  As a result, Carter has successfully impersonated Alice to Bob.
...
The statement \textbf{possibly $b_1, b_2, ...$} evaluates each predicate $b_i$
atomically and checks if it holds in at least one execution.
