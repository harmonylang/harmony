...
\begin{figure}
\begin{center}
\includegraphics[width=\textwidth]{figures/queuefix1.pdf}
\end{center}
\caption{Harmony output.}
\label{fig:queuefix1}
\end{figure}
...
in \autoref{fig:qtestseq}.  The concurrent \texttt{put} test of
...
\autoref{fig:queuefix1} shows the Harmony output of running
...
To fix the code, we have to use hand-over-hand locking (\autoref{ch:cds}).
\autoref{fig:queuefix} shows an implementation that uses the same
data structure as \autoref{fig:queuebug} but uses hand-over-hand locking,
