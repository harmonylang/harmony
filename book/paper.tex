...
Method \texttt{main} waits until both \textit{done} flags are set to \texttt{True}.
...
\begin{code}
\begin{verbatim}
#states = 53 diameter = 5
==== Safety violation ====
T0: __init__() [0-4,31-46] 46 { count: 0, done[0]: False, done[1]: False }
T1: incrementer(0) [ 5- 8]  9 { count: 0, done[0]: False, done[1]: False }
T2: incrementer(1) [ 5-15] 15 { count: 1, done[0]: False, done[1]: True  }
T1: incrementer(0) [ 9-15] 15 { count: 1, done[0]: True,  done[1]: True  }
T3: main()         [17-27] 27 { count: 1, done[0]: True,  done[1]: True  }
...
that \texttt{put} causes the queue to be non-empty, and
...
Like C, Java, and Python, Harmony does not have syntactical
support for monitors. Harmony provides support for monitors
...
every possible execution of the application still terminates.
...
the table when they want to leave.
...
\end{enumerate}
Similarly, \texttt{enter}() blocks until $n$ threads have invoked it.
If more threads try to \texttt{enter}(), they will block until the
previous $n$ threads have invoked \texttt{exit}().
...
The first loop waits while the barrier is fully occupied, while
the second waits for the barrier to fill up.
The $n^\mathit{th}$ thread to arrive at the barrier notifies the
$n-1$ threads that are waiting in the second loop.

Method \texttt{exit}() counts how many threads have left the
barrier.  When the last one leaves, it notifies all the threads
that are waiting in the first loop.
...
\hline
\texttt{any $s$, all $s$} & disjunction / conjunction for set $s$ \\
\hline
\end{tabular}
\end{center}
...
is that a singleton lists in Harmony must contain a comma.  For
...
bytecode.  When you define a method or a lambda function, you are
creating a constant of the program counter type.
Note, however, that labels are currently just integers.  This may
...
\hline
\texttt{$e$ => $e$} & two booleans (implication) \\
\hline
\texttt{not $e$} & a boolean \\
\hline
\texttt{-$e$, \string~$e$, abs($e$)} & an integer \\
\hline
\texttt{$e$ + $e$ + ...} & two or more integers, tuples, or lists \\
\hline
\texttt{$e$ - $e$} & two integers or sets \\
\hline
\texttt{$e$ \& $e$ \& ..., $e$ | $e$ | ..., $e$ \string^ $e$ \string^ ...} & two or more integers or sets \\
\hline
...
\hline
\texttt{get\_context $e$} & $e$ must be \texttt{()} \\
\hline
...
\hline
Bag & pop $n$ and $n$ values and push a bag \\
\hline
