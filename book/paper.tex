...
\newcommand{\cxllink}[1]{
    [\href[pdfnewwindow=true]{https://www.cs.cornell.edu/home/rvr/cxl/code/#1}{#1}]
}
...
\cxlsource{triangle}
\end{code}
\caption{\cxllink{triangle.cxl} Computing triangle numbers.}
\label{fig:triangle}
\end{figure}
...
\cxlsource{Up}
\end{code}
\caption{\cxllink{Up.cxl} Incrementing twice in parallel.}
\label{fig:inc}
\end{figure}
...
\caption{The
\href[pdfnewwindow=true]{https://www.cs.cornell.edu/home/rvr/cxl/output/Up.html}{HTML output of running CXL on Figure~\ref{fig:inc}}.
...
\cxlsource{UpEnter}
\end{code}
\caption{\cxllink{UpEnter.cxl} Broken attempt at fixing the code of Figure~\ref{fig:inc}.}
\label{fig:incenter}
\end{figure}
...
\end{code}
\caption{\cxllink{csbarebones.cxl} A bare bones critical section with two processes.}
...
\cxlsource{cs}
\end{code}
\caption{\cxllink{cx.cxl} CXL model of a critical section.}
\label{fig:cs}
\end{figure}
...
\cxlsource{naiveLock}
\end{code}
\caption{\cxllink{naiveLock.cxl} Na\"{\i}ve implementation of a shared lock.}
\label{fig:uplock}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{figures/naiveLock.png}
\caption{The \href[pdfnewwindow=true]{https://www.cs.cornell.edu/home/rvr/cxl/output/naiveLock.html}{HTML output of running CXL on Figure~\ref{fig:uplock}}.}
...
\end{code}
\caption{\cxllink{naiveFlags.cxl} Na\"{\i}ve use of flags to solve mutual exclusion.}
\label{fig:upflags}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{figures/naiveFlags.png}
\caption{The \href[pdfnewwindow=true]{https://www.cs.cornell.edu/home/rvr/cxl/output/naiveFlags.html}{HTML output of running CXL on Figure~\ref{fig:upflags}}.}
...
\cxlsource{naiveTurn}
\end{code}
\caption{\cxllink{naiveTurn.cxl} Na\"{\i}ve use of turn variable to solve mutual exclusion.}
\label{fig:upturn}
\end{figure}
...
\cxlsource{Peterson}
\end{code}
\caption{\cxllink{Peterson.cxl} Peterson's Algorithm}
\label{fig:peterson}
\end{figure}
...
\end{code}
\caption{\cxllink{PetersonInductive.cxl} Peterson's Algorithm with Inductive Invariant}
...
\cxlsource{csonebit}
\end{code}
\caption{\cxllink{csonebit.cxl} Mutual exclusion using a flag per process.}
\label{fig:csonebit}
\end{figure}
...
\end{code}
\caption{\cxllink{PetersonMethod.cxl} Peterson's Algorithm accessed through methods.}
...
\cxlsource{spinlock}
\end{code}
\caption{\cxllink{spinlock.cxl} Mutual Exclusion using a ``spinlock'' based on test-and-set.}
\label{fig:tas}
\end{figure}
...
\end{code}
\caption{\cxllink{spinlockInv.cxl} Checking invariants.}
\label{fig:tasinv}
\end{figure}
...
\cxlsource{atm}
\end{code}
\caption{\cxllink{atm.cxl} Withdrawing money from an ATM.}
\label{fig:atm}
\end{figure}
...
\cxlsource{csTAS}
\end{code}
\caption{\cxllink{csTAS.cxl} Fixed version of Figure~\ref{fig:uplock} using test-and-set.}
\label{fig:tas2}
\end{figure}
...
\cxlsource{UpLock}
\end{code}
\caption{\cxllink{UpLock.cxl} Program of Figure~\ref{fig:inc} fixed with a lock.}
\label{fig:incfixed}
\end{figure}
...
\cxlsource{RW}
\end{code}
\caption{\cxllink{RW.cxl} Busy-Waiting Reader/Writer Lock implementation.}
\label{fig:rwbusy}
\end{figure}
...
\cxlsource{RWtest}
\end{code}
\caption{\cxllink{RWtest.cxl} Test code for Figure~\ref{fig:rwbusy}.}
...
\cxlsource{RWbusychk}
\end{code}
\caption{\cxllink{RWbusychk.cxl} Checking for Busy Waiting.}
\label{fig:rwblock}
\end{figure}
...
\cxlsource{RWlock}
\end{code}
\caption{\cxllink{RWlock.cxl} Reader/Writer with Two Locks.}
\label{fig:rw2lock}
\end{figure}
...
\cxlsource{RWmulti}
\end{code}
\caption{\cxllink{RWmulti.cxl} Checking that multiple readers can acquire the read lock.}
\label{fig:rwmulti}
\end{figure}
...
\end{code}
\caption{\cxllink{DinersSema.cxl} Italian Philosophers.}
...
\cxlsource{qsort}
\end{code}
\caption{\cxllink{qsort.cxl} Iterative qsort() implementation.}
\label{fig:qsort}
\end{figure}
...
\cxlsource{qsorttest}
\end{code}
\caption{\cxllink{qsorttest.cxl} Test program for Figure~\ref{fig:qsort}.}
\label{fig:qsorttest}
\end{figure}
...
\cxlsource{BBsema}
\end{code}
\caption{\cxllink{BBsema.cxl} Bounded Buffer implementation using semaphores.}
...
\end{code}
\caption{\cxllink{BBsematest.cxl} Test program for Figure~\ref{fig:boundedbuffer}.}
\label{fig:bbtest}
\end{figure}
...
\end{code}
\caption{\cxllink{BBsemadata.cxl} Test whether the bounded buffer delivers the right data in the
right order.}
...
\cxlsource{gpu}
\end{code}
\caption{\cxllink{gpu.cxl} A thread-unsafe GPU allocator.}
\label{fig:gpu}
\end{figure}
...
\cxlsource{RWsbs}
\end{code}
\caption{\cxllink{RWsbs.cxl} Reader/Writer Lock implementation using Split Binary Semaphores.}
...
\end{code}
\caption{\cxllink{RWfair.cxl} Reader/Writer Lock SBS implementation addressing fairness.}
\label{fig:RWfair}
\end{figure}
...
\end{code}
\caption{\cxllink{RWqueue.cxl} Reader/Writer Lock SBS implementation using a queue.}
\label{fig:RWqueue}
\end{figure}
...
\cxlsource{RWqtest}
\end{code}
\caption{\cxllink{RWqtest.cxl} Test program for Figure~\ref{fig:RWqueue}.}
\label{fig:RWqtest}
\end{figure}
...
\cxlsource{RWcv}
\end{code}
\caption{\cxllink{RWcv.cxl} Reader/Writer Lock implementation using Mesa-style condition variables.}
\label{fig:RWcv}
\end{figure}
...
\cxlsource{Diners}
\end{code}
\caption{\cxllink{Diners.cxl} Dining Philosophers.}
\label{fig:diners}
\end{figure}
...
\cxlsource{DinersCV}
\end{code}
\caption{\cxllink{DinersCV.cxl} Dining Philosophers that grab both forks at the same time.}
\label{fig:dinerscv}
\end{figure}
...
\end{code}
\caption{\cxllink{DinersAvoid.cxl} Dining Philosophers that carefully avoid getting into a deadlock
scenario.}
...
\cxlsource{bank}
\end{code}
\caption{\cxllink{bank.cxl} Bank accounts.}
\label{fig:bank}
\end{figure}
...
\cxlsource{actor}
\end{code}
\caption{\cxllink{actor.cxl} A producer/consumer actor.}
\label{fig:actor}
\end{figure}
...
\cxlsource{actortest}
\end{code}
\caption{\cxllink{actortest.cxl} Test code for producer/consumer actor.}
\label{fig:actortest}
\end{figure}
...
\cxlsource{abp}
\end{code}
\caption{\cxllink{abp.cxl} Alternating Bit Protocol.}
\label{fig:abp}
\end{figure}
...
\cxlsource{abptest}
\end{code}
\caption{\cxllink{abptest.cxl} Test code for alternating bit protocol.}
\label{fig:abptest}
\end{figure}
...
\cxlsource{hw}
\end{code}
\caption{\cxllink{hw.cxl} Non-blocking queue.}
\label{fig:hw}
\end{figure}
...
\cxlsource{lst1}
\end{code}
\caption{\cxllink{lst1.cxl} List with non-blocking read operations, part 1.}
\label{fig:lst1}
\end{figure}
...
\cxlsource{lst2}
\end{code}
\caption{\cxllink{lst2.cxl} List with non-blocking read operations, part 2.}
\label{fig:lst2}
\end{figure}
...
\cxlsource{barrier}
\end{code}
\caption{\cxllink{barrier.cxl} Barrier synchronization with semaphores.}
\label{fig:barrier}
\end{figure}
