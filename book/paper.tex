...
This first successfull acquires $q$$\rightarrow$$\texttt{lock}$.
It then finds out that $q$$\rightarrow$$\texttt{next}$ points to
$\texttt{alloc}.\textit{pool}[0]$.  It sets \textit{node} to
...
We can start by single stepping through the
...
\autoref{fig:queuebroken} where it's trying to acquire
