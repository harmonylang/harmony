...
If you run the program through Harmony, it will find not problems.
...
The program then spawns three threads.
...
code/Up.hny:8     assert count == 2, count;
   22 ReadonlyInc
   23 AtomicInc
   24 Load count
   25 Push 2
...
program counter to 32 (skipping over the code for \texttt{incrementer} method).
...
The program got to the faulty execution in 4 ``steps.''
...
Each step shows a thread identifier, the method the thread is executing,
...
the method that is being evaluated and the values of its method variables.}
\label{fig:inchtml1}
\end{figure}

The four steps in the execution are as follows:
...
Underneath it, a table shows the four steps in the execution.
...
executed instruction.  We call this the \emph{time line}.
You can click on such a block to see the state of the Harmony
virtual machine just after executing that instruction.
The step that is being executed is highlighted in green.
The table also lists the program counter of the thread at each step,
...
another thread has updated the shared state;
...
Next to the status, the table shows the \emph{stack trace} of the
thread.  A stack trace is a list of methods that are being invoked.
In this case, the \texttt{implementer} method does not invoke any
...
clicking on one of the blocks in the time lines.
There are also various handy keyboard shortcuts:
\begin{itemize}
\item \emph{Right arrow}: go to the next instruction;
\item \emph{Left arrow}: go to the previous instruction;
\item \emph{Down arrow}: go to the next step;
\item \emph{Up arrow}: go to the previous step;
\item \emph{Enter (aka Return)}: go to the next line of Harmony code;
\item \emph{0}: go to the initial state.
\end{itemize}
...
\item What if we moved line~5 of \autoref{fig:incenter} to after the \textbf{if}
    statement (between lines~8 and~9)?  Do you think that would work?  Run it through
...
in Harmony.\footnote{Actually, there is a third type called
...
non-atomic to allow for efficient caching and other optimizations.
So Peterson's algorithm would not even work on such processors.
Instead, multi-core processors provide
...
list.  Recall that the context of a thread contains
...
separate locks for the head and tail, allowing a \texttt{put} and a \texttt{get} operation
...
Harmony provides support for monitors
...
\begin{tabular}{|l|l|}
\hline
\texttt{len $e$} & cardinality \\
\hline
\texttt{$e$ - $e$} & set difference \\
\hline
\texttt{$e$ \& $e$ \& ...} & intersection \\
\hline
\texttt{$e$ | $e$ | ...} & union \\
\hline
\texttt{$e$ \string^ $e$ \string^ ...} & inclusion/exclusion (elements in odd number of sets) \\
\hline
\texttt{choose $e$} & select an element (Harmony will try all) \\
\hline
\texttt{min $e$} & minimum element \\
\hline
\texttt{max $e$} & maximum element \\
\hline
\end{tabular}
\end{center}
...
loops) and filtering (using the \texttt{such that} keywords).
...
\texttt{dict\{ f($v$) for $v$ in $s$ \}}.  This creates a dictionary
that maps each element $v$ in $s$ to $\mathtt{f}(v)$.
...
Harmony has native support for bags (aka multisets), which are sets
that can contain multiple copies of the same element.
It is represented by a dictionary that maps each element to its
multiplicity.
So \texttt{bag\{ 10, 10, 12 \} = dict\{ 10:2, 12:1 \}}.
...
Bags also support comprehension.
...
variable refering to the same lock in Harmony, you would write:
