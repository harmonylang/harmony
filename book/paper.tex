...
\begin{problems}
\item
Run \autoref{fig:incfixed} using (i) <{synch}> and (ii) \texttt{synchS}.
...
interface for both the <{synch}> and \texttt{synchS} modules.
This interface is like <{acquire}>(\textit{b}) but never blocks.  It
...
\texttt{Queue()} returns the initial value for a queue object
...
checks concurrent <{put}> operations using
<{N}> threads.  Thread \texttt{putter(<{v}>)}
adds <{v}> to the queue.
Thread \texttt{main()} waits until all \texttt{putter}
threads have finished, and then dequeues <{N}>
...
\end{code}
\caption{\harmonylink{code/qtest2.hny} Concurrent <{get()}>}
\label{fig:qtest2}
\end{figure}

Similarly, \autoref{fig:qtest2} shows a program that
checks concurrent <{get}> operations using
<{N}> threads.
...
same entry.
Thread \texttt{main()} waits until all <{getter}>
...
<{get()}> and \texttt{put()} operations}
\label{fig:qtest3}
\end{figure}

\autoref{fig:qtest3} checks a mixture of
concurrent <{get}> and <{put}> operations.
It starts <{N getter}> threads and
<{N}> \texttt{putter} threads.
Thread <{main}> waits until all have finished,
...
The <{get()}> method of the queue implementation
uses a local variable <{node}>, declared using
a <{let}> statement, but it would be easy to
mistakenly use a global variable <{node}> instead.
...
tests the <{get}> method for use of global
...
To this end, it invokes <{P}>(\textit{n\_empty}), which waits until
...
\item <{nwriters}>: the number of writers in the critical section;
...
First the method acquires the <{mutex}>.
...
1 through <{N}>.  Method \texttt{gpuAlloc()} returns the identifier
...
<{wait}> increments the condition's counter, releases the monitor mutex,
...
operation <{notify}> rather than \texttt{signal}.
\index{notify}%
When a thread <{p}> invokes <{notify}>, it does not immediately pass 
...
(by calling either <{release}> or <{wait}>).
...
<{wait}> adds the context of the thread to the bag.
...
<{notify}> removes an arbitrary context from
...
by <{unlock}> followed by <{lock}>.
This turns the solution into a busy-waiting one, inefficient but still
correct.

In \texttt{read\_release}, notice that <{notify}>(?\textit{w\_cond})
...
\texttt{write\_release} executes <{notifyAll}>(?\textit{r\_cond})
as well as <{notify}>(?\textit{w\_cond)}.  Because we
...
\item <{acquire}>(?\textit{tl}) \# \emph{acquire a try lock}
\item \texttt{tryAcquire}(?\textit{tl}) \# \emph{attempt to acquire a try lock}
\item <{release}>(?\textit{tl}) \# \emph{release a try lock}
\end{enumerate}
\noindent
...
The array to be sorted is stored in shared variable <{textqs.arr}>.
...
need to have \textbf{if} statements nested inside the <{while}> loop
...
using these stages.
Method~\texttt{enter}() has two <{wait}> loops, one for each
waiting condition.
...
from a majority ($<{N}> - \texttt{F}$ or equivalently
...
we will assume there are $<{N}> = 3\texttt{F} + 1$ processors.
...
$<{N}> - \texttt{F}$ messages, update their state based on the messages, and
broadcast messages containing their new state.
The reason that a processor waits for $<{N}> - \texttt{F}$ rather than <{N}>
...
We call a set of $<{N}> - \texttt{F}$ processors a \emph{quorum}.
...
In Line~22, a processor waits for $<{N}> - \texttt{F}$ messages using
...
for the given round that have size $k = <{N}> - \texttt{F}$.  The method uses a
...
Note that $<{N}> - \texttt{F} = 2\texttt{F} + 1$, so that the number of
...
$<{N}> - \texttt{F}$ matching \texttt{2.B} responses from the
...
\textbf{from} <{m}> \textbf{import} \texttt{f}.
...
<{let}> statement.
The statement <{let}> $a = b: S_1; S_2, ...$ evaluates <{b}>, assigns
the result to read-only variable <{a}>, and evaluates statements
$S_1, S_2, ...$.  <{let}> supports pattern matching, so you can
write <{let}> $x, (y, z) = b: S_1; S_2, ...$.  This will only work
...
<{let}> $a_1 = b_1$ <{let}> $a_2 = b_2: S_1; S_2; ...$.
...
The statement \textbf{print <{e}>} evaluates <{e}>
...
\hline
<{acquire}>(\textit{bs}) & acquire binary semaphore \texttt{!}\textit{bs} \\
\hline
<{release}>(\textit{bs}) & release binary semaphore \texttt{!}\textit{bs} \\
\hline
<{Condition}>() & return a condition variable \\
\hline
<{wait(c}>, \textit{lk}) & wait on condition variable \texttt{!}<{c}>
and lock \textit{lk} \\ \hline
<{notify(c}>) & notify a thread waiting on condition variable \texttt{!}<{c}> \\
\hline
<{notifyAll(c}>) & notify all threads waiting on condition variable \texttt{!}<{c}> \\
\hline
<{Semaphore(cnt}>) & return a counting semaphore initialized to <{cnt}> \\
\hline
<{P(sema}>) & procure \texttt{!}<{sema}>  \\
\hline
<{V(sema}>) & vacate \texttt{!}<{sema}>  \\
\hline
