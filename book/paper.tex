...
There are also various algorithms where read-only access does not
require a lock but updates do require a lock---these are useful in
situations where most accesses are read-only.

TODO: Talk about seqlock perhaps.
...
\begin{tabular}{|l|l|}
\hline
\texttt{rec\_alloc()} & allocate a record \\
\hline
\texttt{rec\_free(r)} & free record \texttt{r} \\
\hline
\end{tabular}
\vspace{1em}

CXL supports an address value \texttt{NULL} that can be
used as a null pointer.  \texttt{rec\_alloc} returns \texttt{NULL}
...
variable name: if omitted the top of the stack must contain the path
name of the variable as an AddressValue.
...
of the stack.  If the name is omitted, the path name is underneath that
value on the stack.
