...
atomic swap (similar to \autoref{ex:swap}).
\item
...
Implement counting semaphores using split binary semaphores.
...
a \texttt{downgrade} method.  Note, you cannot do this simply by releasing the
write lock and then obtaining the read lock, because the process needs
