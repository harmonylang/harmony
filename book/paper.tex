...
\mathit{turn} = 1 - i$, that is, the negation of the condition on the \textbf{await}
...
\vspace{1em}
For Python users: note that $s <= t$ does not check if $s$ is a subset of
...
of the method or when it hits an exception.  Exceptions include divide by zero,
