...
[TODO.  The following must be updated.]
CXL does not complain about Figure~\ref{fig:rwbusy}, but using
the right test, CXL can be used to check for busy-waiting.
One approach is as follows.  First, obtain a lock in the initialization
code by adding the following line to the end of the code:
\begin{code}
\begin{verbatim}
call acquire_rlock();
\end{verbatim}
...
Second, run CXL with the \texttt{-b} flag.  This flag tells CXL to check
