...
For this, each shared variable has an \emph{address}.
\index{address}%
...
Because there are no objects in Harmony,
there are no object methods either.
However, methods in Harmony are program counter constants.
...
Harmony does support references to values (\autoref{ch:method}),
...
They can be bracketed either by '(' and ')' or by '[' and ']', and
even the brackets are often optional.  Importantly, with a singleton list, the
...
Harmony also allows pattern matching in defining and invoking methods.
