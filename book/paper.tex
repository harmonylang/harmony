...
\begin{itemize}
\item $n$ readers at \texttt{@rcs} $\Rightarrow \mathtt{nreaders} = n$,
\item $n$ writers at \texttt{@rcs} $\Rightarrow \mathtt{nwriters} = n$,
\item $(\mathtt{nreaders} \ge 0 \land \mathtt{nwriters} = 0) \lor
    (\mathtt{nreaders} = 0 \land 0 \le \mathtt{nwriters} \le 1)$.
\end{itemize}
It is easy to see that the invariants hold and imply the specification.
...
So it is considered ok to have processes busy wait for a lock to become
available, but not to busy wait for application-specific conditions.
