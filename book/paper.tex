...
The <{round}> array is kept to check the correctness of this
...
after <{main()}> acquires the lock but before <{main()}> releases it, the
<{handler}>() method will block trying to acquire the lock, even though
...
Implement a new method \texttt{i\_put}(<{item}>) that does not
...
\begin{itemize}
\item <{inc(p}>): atomically increments \texttt{!}<{p}> and returns
the old value;
\item <{exch(p}>): sets \texttt{!}<{p}> to <{None}> and returns
the old value.
\end{itemize}

Method <{produce(item}>) uses <{inc}>(?<{back}>)
to allocate
...
(<{dst}>, <{id}>, <{found}>).
<{dst}> is the identifier of the destination processor;
<{id}> is the identifier that is being forwarded; and
<{found}> is a boolean indicating the second phase of the protocol.
The \texttt{receive}(<{self}>) method looks for all messages
destined for the processor with identifier <{self}>.
...
Method \texttt{processor}(<{self}>, <{succ}>) is the
code for a processor with identifier <{self}> and successor
<{succ}>.
...
a \texttt{.abort} message, the bank restores <{balance}> first.
...
\texttt{.withdraw} request to <{b1}> and a \texttt{PREPARE}
message containing a \texttt{.deposit} request to <{b2}>.
...
<{uid}>.  Both start by broadcasting a \texttt{.read}
...
\texttt{receive}(<{round}>, <{k}>) method returns all \emph{subbags} of messages
...
In its most basic form, <{if}> $c: S_1; S_2; ...$ evaluates <{c}>
...
The statement <{if}> $c: S_1, S_2, ...$ <{else}>:
...
Application has the highest precedence.  So, \texttt{!}<{a b}> is interpreted as
\texttt{!}(<{a b}>) and <{a b}> \texttt{+} <{c d}> is interpreted as
(<{a b}>) \texttt{+} (<{c d}>).
...
\hline
<{acquire(bs)}> & acquire binary semaphore \texttt{!}<{bs}> \\
\hline
<{release(bs)}> & release binary semaphore \texttt{!}<{bs}> \\
\hline
<{Condition()}> & return a condition variable \\
\hline
<{wait(c}>, <{lk}>) & wait on condition variable \texttt{!}<{c}>
and lock <{lk}> \\ \hline
