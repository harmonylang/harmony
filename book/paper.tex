...
In the first phase, an operation asks each server for its current
...
observed, and client $c'$ needs to create a new timestamp, it can select
$(t + 1, c')$.  After all $(t + 1, c') > (t, u)$ and no other client
...
highest timestamp it saw along with its corresponding value.
...
In case of a server, its timestamp $t$ is initialized to
...
\textit{uid}.  Both start with broadcasting a \texttt{.read}
