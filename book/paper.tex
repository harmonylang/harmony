...
\begin{figure}
\begin{code}
\VerbatimInput[xleftmargin=5mm,numbers=left]{code/RW.cxl}
\end{code}
...
\begin{figure}
\begin{code}
\VerbatimInput[xleftmargin=5mm,numbers=left]{code/RWtest.cxl}
\end{code}
...
\begin{figure}
\begin{code}
\VerbatimInput[xleftmargin=5mm,numbers=left]{code/BBsematest.cxl}
\end{code}
...
\item Bonus question: implement a version of parallel qsort
(see Example~\ref{ex:qsort}) using the actor model.
\end{problems}
...
systems is that the former takes \emph{failures} into account,
...
\section*{Exercises}
\begin{problems}
\item A common pattern in distributed systems is the so-called \emph{client/server}
model, where a server process provides services to a set of clients.  The clients
send request messages to the server, and the server sends a response to each
request.  Develop a protocol for a single client and server using the same network
model as for the ABP protocol.  By way of example, let a request contain an
integer and the response consist of the square of that integer.
\end{problems}
...
\begin{figure}
\begin{code}
\VerbatimInput[xleftmargin=5mm,numbers=left]{code/lst1.cxl}
\end{code}
...
\begin{figure}
\begin{code}
\VerbatimInput[xleftmargin=5mm,numbers=left]{code/lst2.cxl}
\end{code}
...
\item A \emph{seqlock} consists of a lock and a version number.
