...
\index{alloc module}%
support thread-safe (but not interrupt-safe) dynamic allocation of
dictionaries.  There are just two methods:

\vspace{1em}
\begin{tabular}{|l|l|}
\hline
\texttt{malloc()} & allocate a dictionary \\
\hline
\texttt{free($r$)} & free an allocated dictionary $r$ \\
\hline
\end{tabular}
\vspace{1em}
