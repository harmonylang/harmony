...
\chapter{Introduction to Programming with Harmony}
...
\begin{itemize}
\item Syntax: Every statement in Harmony must be terminated by a semicolon
...
Harmony does support \textbf{for} loops and various comprehensions over sets.
...
In fact, Harmony will try all possible interleavings of the processes
...
is easy to translate Harmony programs into Python, but on the other hand,
it is easier to find concurrency bugs using Harmony.
...
A Harmony program is translated
into \emph{bytecode}, which is list of machine instructions that the
Harmony virtual machine (HVM) executes.
...
\harmonysource{cs}
\end{code}
\caption{\harmonylink{code/csbarebones.hny} Harmony model of a critical section.}
\label{fig:cs}
\end{figure}
...
For this (as well as some other uses), Harmony supports \emph{pointers}
\index{pointer}%
to shared variables.
...
You can test the code by running \texttt{harmony -m RW=RWlock RWtest}.
...
divide-by-zero.  An interrupt is caused by some peripheral device.
\index{exception}

Harmony allows modeling interrupts using operator \texttt{trap}:
\begin{code}
...
read \textit{count} (and thus still has value 0) but before \textit{main()} writes the
...
it is executed the same process that holds the lock.
...
The important rule to remember is for a process to disable interrupts \emph{before}
attempting to acquire a lock.

Try moving \texttt{lock}() to the beginning of \texttt{increment}()
...
\section*{Exercises}
\begin{problems}
\item Make methods \texttt{produce}() and \texttt{consume}() in \autoref{fig:boundedbuffer}
both thread-safe and interrupt-safe.
\end{problems}
...
\hline
\texttt{$e$ < $e$, $e$ <= $e$} & any two Harmony values \\
\hline
\texttt{$e$ > $e$, $e$ >= $e$} & any two Harmony values \\
\hline
...
% I wrote the Harmony compiler and this book in May and June of 2020,
