...
You can think of the \texttt{choose} operator as enumerating all possible
...
(Also, you do not get to use the \texttt{atomically} keyword
or a \emph{lock}, yet.)
\end{problems}
...
preceding it with a dot.  So \texttt{.example} is the same string
as \texttt{"example"}.
...
\item \texttt{Behavior Violation}:  The program terminated in a state
not allowed by the behavioral specification.
...
reported.
You will learn about behavioral violations in \autoref{ch:testing}.
...
because <{count}> has value 1 instead of 2.
...
sets shared variable <{count}> to 0 and
shared variable <{done}> to \texttt{[False, False]}.
\item Thread <{incrementer(0)}> (with identifier T1)
...
\item Thread <{incrementer(0)}>
continues execution, executing instructions 10 through 28
storing value 1 into <{count}> (instruction 10),
storing <{True}> into <{done[0]}> (instructions 11 through 15)
finding that <{done[1]}> is <{True}> (instructions 16 through 21),
...
where $L$ is the name of the label (in this case, \texttt{cs}).
...
Initially the lock is not owned, indicated by \textit{lockTaken} being <{False}>.
To enter the critical section, a thread waits until \textit{lockTaken} is <{False}>
...
releases the lock by setting \textit{lockTaken} back to <{False}>.
...
<{True}> in \textit{lockTaken} in instruction~9, thread~1 gets
to run.
(Recall that you can hover your mouse over a machine instruction in order
to see what it does.)
Because \textit{lockTaken} is still <{False}>, it too
believes it can acquire the lock, and stores <{True}> in
\textit{lockTaken} and moves on to the critical section.
Finally, thread~0 moves on, also stores <{True}>
...
suffers from the same sort of race condition as the <{count}> variable
...
critical section, then \textit{flags}[<{i}>] = <{True}>.
...
$t_1 > t_0$, it finds that \textit{flags}[1] = <{False}>.
Because of the invariant, \textit{flags}[1] = <{False}> implies that
...
between $t_1$ and $t_2$, because \textit{flags}[1] = <{False}> at
time $t_1$.  To reach the critical section between $t_1$ and $t_2$,
it would first have to set \textit{flags}[1] to <{True}> and
then wait until \textit{flags}[0] = <{False}>.  But that does not happen
until time $t_2$.
...
to <{True}> and \textit{turn} to $1 - \mathit{self}$) can be reversed.
...
if you define \texttt{FIFO} to be <{True}>.  Moreover, if you define
\texttt{FIFO} to be <{False}>, it will find a scenario for the
...
(It is not used if \texttt{FIFO} is <{True}>.)
...
\begin{itemize}
\item If the list is empty, $q$$\rightarrow$$\mathtt{tail}$ should be <{None}>.
...
$q$$\rightarrow$$\texttt{tail}$$\rightarrow$$next$ should be <{None}>;
...
\texttt{NOPS} threads and each choose to do either a \texttt{put}
or a \texttt{get} operation.  In case of a \texttt{put} operation,
...
\texttt{get}(?$q$).
% Looking in the top green row, we see that the value of $q$ is
% \{ \texttt{.lock}: <{True}>, \texttt{.next}: <{None}>, \texttt{.value}: <{None}> \}.
T1 is currently executing
\texttt{acquire}(\texttt{?alloc}.\textit{pool}[0].\texttt{lock})
within \texttt{put}(?$q$, 2), but 
...
\texttt{put}(1) (see the output column for that clue: the thread printed that).
...
\texttt{put}(?$q$, 2).  We can see it got as far as putting 2 on the queue,
...
\texttt{get}($?q$).  We can also see that it freed \textit{pool}[0],
...
More precisely, T2 was executing \texttt{put}(?$q$, 2), when T3
preempted it with \texttt{get}(?$q$) and removed the node that T2
...
T2 first calls \texttt{put}(?$q$, 1) and then allocates a new
...
\textit{nq} is initialized to~?$q$.  T2 then enters into the
\textbf{while} loop and tries to acquire
$\mathit{nq}$$\rightarrow$$\mathtt{lock}$.  This succeeds, and
next T2 executes \textbf{let} $n = \mathit{nq}$$\rightarrow$$\mathtt{next}$.
Now $n =\,?\mathtt{alloc}.\mathit{alloc}[0]$, which is not <{None}>.
It then releases $\mathit{nq}$$\rightarrow$$\mathtt{lock}$
(\textit{nq} points to $q$).  It then sets \textit{nq} to $n$, which
is still $?\mathtt{alloc}.\mathit{alloc}[0]$.  Finally, it calls
$\mathtt{acquire}(?\mathit{nq}$$\rightarrow$$\mathtt{lock})$.
But before it can complete that operation, T3 runs next.

T3 chooses "get" and then goes on to invoke $\mathtt{get}(?q)$.
This first successfully acquires $q$$\rightarrow$$\texttt{lock}$.
T3 then finds out that $q$$\rightarrow$$\texttt{next}$ points to
$\texttt{alloc}.\textit{pool}[0]$.  T3 sets \textit{node} to
$\texttt{alloc}.\textit{pool}[0]$ as well and sets
$q$$\rightarrow$$\texttt{next}$ to 
$\mathtt{node}$$\rightarrow$$\texttt{next}$.
T3 sets <{result}> to 
$\mathtt{node}$$\rightarrow$$\texttt{value}$ (which is 1)
and then frees \textit{node}.  This is where the problem is---T2 is
...
that both the \textit{main}() and \textit{handler}() methods increment <{count}>.
...
reads <{count}> (and thus still has value 0) but before \textit{main()} writes the
...
\hline
String & \texttt{"example"}, \texttt{.example} \\
\hline
...
So, for example, \texttt{<{True}> < 0 < .xyz < \{ 0 \})}.
...
For example, \texttt{.hello} is the same as the string \texttt{"hello"}.
...
\hline
\texttt{print $b$ [, $b$, ...]} & each $b$ is a boolean \\
\hline
...
The statement \textbf{print $e$} atomically evaluates $e$ in read-only
mode and adds the result to the print log.  The print log is used to
...
$S_1, S_2, ...$ after waiting until $c$ evaluates to <{True}>.
...
$S_1, S_2, ...$ repeatedly as long as $c$ evaluates to <{True}>.
Harmony does not support \textbf{break} or \textbf{continue} statements.
