...
\end{verbatim}
\end{code}
\caption{The \texttt{Lock} interface using suspension.}
...
Figure~\ref{fig:rwbusy} has to obtain a spinlock just to check
...
\begin{figure}
\begin{code}
\VerbatimInput[xleftmargin=5mm,numbers=left]{../DinersSema.cxl}
\end{code}
...
\begin{figure}
\begin{code}
\VerbatimInput[xleftmargin=5mm,numbers=left]{../PCsemadata.cxl}
\end{code}
...
\begin{figure}
\begin{code}
\VerbatimInput[xleftmargin=5mm,numbers=left]{../RWbinsem.cxl}
\end{code}
...
\begin{figure}
\begin{code}
\VerbatimInput[xleftmargin=5mm,numbers=left]{../RWfair.cxl}
\end{code}
\caption{Reader/Writer Lock implementation addressing fairness.}
\label{fig:RWfair}
\end{figure}
...
a set of variables and methods that operate on those variabled.
...
\texttt{wait} releases the monitor lock and blocks while trying
...
a process trying to procure it, and then procures the monitor lock.
...
As in Hoare monitors, there is a lock associated with each monitor,
and the lock is acquired upon entry to a method and released upon exit.
Mesa monitors also have condition variable that a process can wait on.
Like in Hoare monitors, the \texttt{wait} operation release the monitor
lock.
The most important difference is in what \texttt{signal} does.
To make the distinction more clear, we shall call corresponding Mesa
operation \texttt{notify} rather than \texttt{signal}.
When a process $p$ invokes \texttt{notify}, it does not immediately pass 
he baton to some process $q$ requiring the baton.  Instead, $p$ holds onto
...
It can choose to either let a process
...
If there are such bedrooms, $p$ selects one and let's that process into
...
do is check to see if the condition holds.  If not, it should go back
into the bedroom corresponding to the condition.

It turns out that with the Mesa monitor there is another important option:
...
bedrooms into the hall.  Note that called \texttt{notify} on an empty
bedroom is considered a no-op in Mesa.  Mesa also has a function called
...
\begin{figure}
\begin{code}
\VerbatimInput[xleftmargin=5mm,numbers=left]{../RWcv.cxl}
\end{code}
\caption{Unfair Reader/Writer Lock implemtation using Mesa-style condition variables.}
\label{fig:RWcv}
\end{figure}
