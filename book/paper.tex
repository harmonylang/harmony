...
and \emph{progress}.  The former is an example of
\texttt{safety property}---it prevents something ``bad'' from
happening, like a reader and writer process both entering the
critical section.  The \emph{progress} property is an example
of a liveness property---that something good eventually happens.
...
successful and enter.  After the process leaves, it immediately
...
indefinitely.  So even if neiter reader was in the critical section
for all of the time, the writer never had a chance.
In this section we will present a version of a reader/writer lock
