...
For readability, it sometimes helps to write $a(b)$ for function application and
...
$a + b + c$.  However, $a - b - c$ is illegal, as is any combination of operators such
as $a + b < c$.  In such cases you have to add parentheses or brackets to indicate what
...
exception is the expression $a$ \textbf{if} $c$ \textbf{else} $b$, where $b$ is evaluated
...
Harmony generalizes this construct for any unary operator besides \textbf{not}.
So $1 - + 2$ equals $-3$ and $a$ \textbf{not or} $b$ is the same as
\textbf{not} ($a$ \textbf{or} $b$).
