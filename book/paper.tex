...
services acts as if it were a single one.  This requires that the
...
to work even if some subset of replicas crash.
...
Each replica monitors its direct predecessor in the chain.  If it has
no predecessors, we call it the \emph{head}.  The head may change over
time, as a head may crash and replaced by another.
...
of the \emph{tail} replica (the replica that has no live successors),
...
This is sometimes called the \emph{Fail-Stop} failure model~\cite{SS83},
...
0 through $\mathtt{NREPLICAS} - 1$.  At least one of the replicas 
must be immortal---the code selects one of them although none of the
replicas is required to fail during execution.
In practice, of course, a replica does not know whether it is immortal
or not---it should just assume so.  The immortality is only used for
modeling the system.
...
evaluate to \textbf{False} in case $\textit{immortal}$ is false.
...
it is the head at any time.  Moreover, when it has
...
this message on the network.  The other form of payload is a tuple
...
Instead, in the model, we add \textit{self} to the history until the
history have \texttt{NOPS} operations on it or until the head crashes,
whichever comes first.  The head also broadcasts this update, which
is intended for its successor.  Because of failures, it may be that
more than one replica is a successor of the head at different times.

Otherwise, the replica awaits a message from its direct predecessor.
