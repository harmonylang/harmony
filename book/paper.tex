...
\texttt{AtomicInc}, and \texttt{Choose}.  Also, any instruction after
a \texttt{Stop} instruction starts a macro step.  The CXL machine
...
states and the edges are macro steps.  A terminating state is a
state with an empty context bag or a state where all contexts are
blocked.
