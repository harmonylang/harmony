...
\item \autoref{ch:2pc} describes the two-phase commit protocol used to implement
...
stored in them.  Making matters more complicated, they store data on disk, but
we will ignore this in this book.
The complication we address here is that databases may be \emph{sharded},
where different parts of the data are stored on different servers with
different disks.  The different servers may even be under different
authoritive domains, such as multiple banks maintaining the accounts of their
clients.

In database terminology, a \emph{transaction} is an operation on the database.
The operation can be quite complex.  A transaction should be executed with at
...
For simplicity we will assume a collection of banks, each maintaining a
single account.  There are two kinds of transactions: \emph{transfer}
(similar to \autoref{ex:bank}) and \emph{inquiry}.  In this example, a transfer
is a transaction that moves some balance between two accounts.
An inquiry is a transaction over all accounts and checks that the sum of the
...
inquiry transaction may accidentally witness a state in which either
...
to prepare to commit to a transaction.  A server can either respond with
...
or \texttt{NO} if it does not want to participate in the transaction.
...
servers that responded with \texttt{YES} (if any) must be notified in
either case to inform them of the decision.
