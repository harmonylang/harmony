...
one of the variables is \texttt{False}.
...
section if \texttt{private[$i$] == False} and no other process modified
\texttt{private[$i$]}.
Indeed, there is no need to keep \texttt{private} as a shared
variable---we only did so to be able to show the invariants.
...
In CXL, atomic statements allow you to implement your own
low synchronization primites like test-and-set.  Atomic statements
