...
does not block (see \autoref{ch:condwait});
...
operation (see \autoref{ch:synch}).
...
cannot implement Peterson's algorithm in those languages very easily.

\glsadd{interlock instruction}%
For proper synchronization, multi-core processors provide so-called
\emph{interlock instructions}: \index{interlock instruction}%
...
\emph{data race}.
In Harmony, a data race has a specific meaning: a state in which multiple
...
However, for the same reason that you should not use \textbf{atomic}
...
programming languages.  But both can be very useful for testing.
