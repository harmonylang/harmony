...
\newglossaryentry{blocked process}
{
  ...
  description={is a process cannot make progress without the help of another process.
...
\newglossaryentry{busy waiting}
{
  ...
  description={(aka spin-waiting) is when a process waits in a loop for some
      application-defined condition while changing the state, for example by
      continually acquiring and releasing a lock}
}
...
\newglossaryentry{machine instruction}
{
  ...
  description={is an atomic operation on the CXL machine, executed by a process}
}
...
\newglossaryentry{shared variable}
{
  ...
  description={is a variable that is stored in the memory of the CXL machine and
...
{
  name=state,
  description={an assignment of values to variables.  In a CXL machine, this includes
...
\item Chapters~\ref{ch:cxlvalues} and~\ref{ch:cxlmachine} present some more
details of the CXL language and machine to understand the problem
underlying concurrency better.
\item Chapter~\ref{ch:critical} introduces the concept of a critical
section, and Chapter~\ref{ch:naive} presents various flawed implementations
...
elegant implementation of a critical section.
...
\index{label}
identifying a location in the CXL machine code.  The first thing we need to
...
We suggested the last technique to address the deadlock issue in the Dining
Philosophers problem (Chapter~\ref{ch:deadlock}).
...
The code demonstrates an important technique to use when waiting for multiple
conditions.
...
The next approach suggested by Havender is to allow processes to back out.
...
The approach to order the resources was already presented in Chapter~\ref{ch:deadlock}
