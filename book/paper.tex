...
resource they need.
\item Chapters~\ref{ch:monitors} and~\ref{ch:mesa} present
...
\item Suppose a sports club has only one dressing room.  The policy is that
either only women or only men may be in the dressing room at any time.
Implement a ``dressing room lock'' that allows multiple people to enter the
dressing room while observing the policy.
\end{enumerate}
...
Note that it uses a shared variable \texttt{a} containing the array to be
sorted and a shared variable \texttt{todo} containing the ranges of the
...
worker to terminate.  After that it should check if the array is sorted and a
