...
\newcommand{\cxllink}[1]{%
[\href{https://www.cs.cornell.edu/home/rvr/cxl/#1}{\underline{#1}}]%
}

\newcommand{\cxlref}[2]{%
\href{https://www.cs.cornell.edu/home/rvr/cxl/output/#1}{\underline{#2}}%
}
...
\begin{itemize}
\item \autoref{ch:cxlintro} introduces the CXL programming
...
\item \autoref{ch:cxlmachine} presents the
...
is not trivial.
\item \autoref{ch:peterson} introduces Peterson's Algorithm, an
elegant solution to implementating a critical section.
\item \autoref{ch:method} gives some more details on the CXL
language.
\item \autoref{ch:spinlock} introduces ``hardware'' locks
for implemented critical sections.
\item \autoref{ch:synch} presents CXL modules
...
\autoref{ch:rdwrbusy} also talks about busy-waiting, which is
...
\cxlsource{triangle}
\end{code}
\caption{\cxllink{code/triangle.cxl} Computing triangle numbers.}
\label{fig:triangle}
\end{figure}

\autoref{fig:triangle} shows a simple example of a CXL program.
...
you initialize it to 1.  (You do not need to understand the error report at this time.  They will be explained in more detail in \autoref{ch:cxlmachine}.)
...
\cxlsource{Up}
\end{code}
\caption{\cxllink{code/Up.cxl} Incrementing twice in parallel.}
\label{fig:inc}
\end{figure}
...
machine.  The details of this appear in \autoref{ch:cxlmachine},
but suffice it to
...
\end{code}
\caption{\cxllink{python/Up.py} Python implementation of \autoref{fig:inc}.}
\label{fig:incpy}
\end{figure}
...
\end{code}
\caption{\cxllink{python/UpMany.py} Incrementing \texttt{N} times.}
\label{fig:incmany}
\end{figure}
...
\end{verbatim}
\end{code}
\caption{The first part of the CVM bytecode corresponding to \autoref{fig:inc}.}
\label{fig:inccode}
\end{figure}
...
\end{verbatim}
\end{code}
\caption{The text output of running CXL on \autoref{fig:inc}.}
\label{fig:incoutput}
\end{figure}

\glsadd{stack machine}%

Compiling the code in \autoref{fig:inc} results in the CVM bytecode
...
\texttt{Up.cxl} is as follows (see \autoref{fig:inccode}):
...
instructions.
\autoref{fig:incoutput} shows the output produced by running CXL on the
\texttt{Up.cxl} program.

\begin{figure}
\includegraphics[width=\textwidth]{figures/Up1.png}
\caption{The
\cxlref{Up.html}{HTML output of running CXL on \autoref{fig:inc}}.
...
\item \autoref{fig:incenter} shows an attempt at trying to fix the code of
    \autoref{fig:inc}.  Run it through CXL and see what happens.  Based on
...
\cxlsource{UpEnter}
\end{code}
\caption{\cxllink{code/UpEnter.cxl} Broken attempt at fixing the code of \autoref{fig:inc}.}
\label{fig:incenter}
\end{figure}
...
\end{code}
\caption{\cxllink{code/csbarebones.cxl} A bare bones critical section with two processes.}
...
\cxlsource{cs}
\end{code}
\caption{\cxllink{code/csbarebones.cxl} CXL model of a critical section.}
\label{fig:cs}
\end{figure}
...
\autoref{fig:csbarebones} shows the CXL code.
...
\cxlsource{naiveLock}
\end{code}
\caption{\cxllink{code/naiveLock.cxl} Na\"{\i}ve implementation of a shared lock.}
\label{fig:uplock}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{figures/naiveLock.png}
\caption{The \cxlref{naiveLock.html}{HTML output of running CXL on \autoref{fig:uplock}}.}
...
\end{code}
\caption{\cxllink{code/naiveFlags.cxl} Na\"{\i}ve use of flags to solve mutual exclusion.}
\label{fig:upflags}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{figures/naiveFlags.png}
\caption{The \cxlref{naiveFlags.html}{HTML output of running CXL on \autoref{fig:upflags}}.}
...
\cxlsource{naiveTurn}
\end{code}
\caption{\cxllink{code/naiveTurn.cxl} Na\"{\i}ve use of turn variable to solve mutual exclusion.}
\label{fig:upturn}
\end{figure}
...
\autoref{fig:naiveLockhtml} shows the CXL output.
...
However, if you run the program through CXL (\autoref{fig:naiveflagshtml}),
...
\section*{Exercises}
\begin{problems}
\item Run \autoref{fig:cs} using CXL.  As there is no protection of the critical
...
in \autoref{fig:cs} and run CXL again.
...
\cxlsource{Peterson}
\end{code}
\caption{\cxllink{code/Peterson.cxl} Peterson's Algorithm}
\label{fig:peterson}
\end{figure}
...
machine works.
In \autoref{ch:cxlmachine} we talked about the concept of \emph{state}:
\index{state}%
...
\end{code}
\caption{\cxllink{code/PetersonInductive.cxl} Peterson's Algorithm with Inductive Invariant}
...
\autoref{fig:petersonproof} formalizes $\mathcal{I}_p(i)$ in CXL.
...
\cxlsource{csonebit}
\end{code}
\caption{\cxllink{code/csonebit.cxl} Mutual exclusion using a flag per process.}
\label{fig:csonebit}
\end{figure}
...
is the same as that for dictionaries (see \autoref{ch:cxlmachine}).
...
\end{code}
\caption{\cxllink{code/PetersonMethod.cxl} Peterson's Algorithm accessed through methods.}
...
\cxlsource{spinlock}
\end{code}
\caption{\cxllink{code/spinlock.cxl} Mutual Exclusion using a ``spinlock'' based on test-and-set.}
\label{fig:tas}
\end{figure}
...
\end{code}
\caption{\cxllink{code/spinlockInv.cxl} Checking invariants.}
\label{fig:tasinv}
\end{figure}

CXL can check these invariants as well.  \autoref{fig:tas} already
...
\begin{enumerate}
\item Fix the code in \autoref{fig:atm}.
...
\cxlsource{atm}
\end{code}
\caption{\cxllink{code/atm.cxl} Withdrawing money from an ATM.}
\label{fig:atm}
\end{figure}
...
\cxlsource{csTAS}
\end{code}
\caption{\cxllink{code/csTAS.cxl} Fixed version of \autoref{fig:uplock} using test-and-set.}
\label{fig:tas2}
\end{figure}
...
\cxlsource{UpLock}
\end{code}
\caption{\cxllink{code/UpLock.cxl} Program of \autoref{fig:inc} fixed with a lock.}
\label{fig:incfixed}
\end{figure}
...
\cxlsource{RW}
\end{code}
\caption{\cxllink{code/RW.cxl} Busy-Waiting Reader/Writer Lock implementation.}
\label{fig:rwbusy}
\end{figure}
...
\cxlsource{RWtest}
\end{code}
\caption{\cxllink{code/RWtest.cxl} Test code for \autoref{fig:rwbusy}.}
...
\cxlsource{RWbusychk}
\end{code}
\caption{\cxllink{code/RWbusychk.cxl} Checking for Busy Waiting.}
\label{fig:rwblock}
\end{figure}
...
CXL does not complain about \autoref{fig:rwbusy}, but, using
...
\cxlsource{RWlock}
\end{code}
\caption{\cxllink{code/RWlock.cxl} Reader/Writer with Two Locks.}
\label{fig:rw2lock}
\end{figure}
...
\cxlsource{RWmulti}
\end{code}
\caption{\cxllink{code/RWmulti.cxl} Checking that multiple readers can acquire the read lock.}
\label{fig:rwmulti}
\end{figure}
...
\end{code}
\caption{\cxllink{code/DinersSema.cxl} Italian Philosophers.}
...
\cxlsource{qsort}
\end{code}
\caption{\cxllink{code/qsort.cxl} Iterative qsort() implementation.}
\label{fig:qsort}
\end{figure}
...
\cxlsource{qsorttest}
\end{code}
\caption{\cxllink{code/qsorttest.cxl} Test program for \autoref{fig:qsort}.}
\label{fig:qsorttest}
\end{figure}
...
\cxlsource{BBsema}
\end{code}
\caption{\cxllink{code/BBsema.cxl} Bounded Buffer implementation using semaphores.}
...
\end{code}
\caption{\cxllink{code/BBsematest.cxl} Test program for \autoref{fig:boundedbuffer}.}
\label{fig:bbtest}
\end{figure}
...
\end{code}
\caption{\cxllink{code/BBsemadata.cxl} Test whether the bounded buffer delivers the right data in the
right order.}
...
\cxlsource{gpu}
\end{code}
\caption{\cxllink{code/gpu.cxl} A thread-unsafe GPU allocator.}
\label{fig:gpu}
\end{figure}
...
\cxlsource{RWsbs}
\end{code}
\caption{\cxllink{code/RWsbs.cxl} Reader/Writer Lock implementation using Split Binary Semaphores.}
...
\end{code}
\caption{\cxllink{code/RWfair.cxl} Reader/Writer Lock SBS implementation addressing fairness.}
\label{fig:RWfair}
\end{figure}
...
\end{code}
\caption{\cxllink{code/RWqueue.cxl} Reader/Writer Lock SBS implementation using a queue.}
\label{fig:RWqueue}
\end{figure}
...
\cxlsource{RWqtest}
\end{code}
\caption{\cxllink{code/RWqtest.cxl} Test program for \autoref{fig:RWqueue}.}
\label{fig:RWqtest}
\end{figure}
...
\cxlsource{RWcv}
\end{code}
\caption{\cxllink{code/RWcv.cxl} Reader/Writer Lock implementation using Mesa-style condition variables.}
\label{fig:RWcv}
\end{figure}
...
\cxlsource{Diners}
\end{code}
\caption{\cxllink{code/Diners.cxl} Dining Philosophers.}
\label{fig:diners}
\end{figure}
...
\autoref{fig:diners} implements the dining philosophers in CXL, using a
...
\cxlsource{DinersCV}
\end{code}
\caption{\cxllink{code/DinersCV.cxl} Dining Philosophers that grab both forks at the same time.}
\label{fig:dinerscv}
\end{figure}
...
\end{code}
\caption{\cxllink{code/DinersAvoid.cxl} Dining Philosophers that carefully avoid getting into a deadlock
scenario.}
...
\cxlsource{bank}
\end{code}
\caption{\cxllink{code/bank.cxl} Bank accounts.}
\label{fig:bank}
\end{figure}
...
\cxlsource{actor}
\end{code}
\caption{\cxllink{code/actor.cxl} A producer/consumer actor.}
\label{fig:actor}
\end{figure}
...
\cxlsource{actortest}
\end{code}
\caption{\cxllink{code/actortest.cxl} Test code for producer/consumer actor.}
\label{fig:actortest}
\end{figure}
...
\autoref{fig:actor} shows a CXL solution to the
...
\cxlsource{abp}
\end{code}
\caption{\cxllink{code/abp.cxl} Alternating Bit Protocol.}
\label{fig:abp}
\end{figure}
...
\cxlsource{abptest}
\end{code}
\caption{\cxllink{code/abptest.cxl} Test code for alternating bit protocol.}
\label{fig:abptest}
\end{figure}
...
\cxlsource{hw}
\end{code}
\caption{\cxllink{code/hw.cxl} Non-blocking queue.}
\label{fig:hw}
\end{figure}
...
\cxlsource{lst1}
\end{code}
\caption{\cxllink{code/lst1.cxl} List with non-blocking read operations, part 1.}
\label{fig:lst1}
\end{figure}
...
\cxlsource{lst2}
\end{code}
\caption{\cxllink{code/lst2.cxl} List with non-blocking read operations, part 2.}
\label{fig:lst2}
\end{figure}
...
\cxlsource{barrier}
\end{code}
\caption{\cxllink{code/barrier.cxl} Barrier synchronization with semaphores.}
\label{fig:barrier}
\end{figure}
...
\autoref{ch:cxlmachine} provides an introduction to CXL values.
