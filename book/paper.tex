...
of processors.  Typically the processors have only limited information.
In the version that we present, each processor has a unique identifier.  The
processors are organized in a ring, but each processor only knows its
own identifier and the identifier of its successor.  We assume that
each processor can reliably send messages to its successor.

The protocol that we present elects as leader the processor with the
highest identifier and works in two phases: in the first
...
receives its own identifier, it knows that it is the leader.  That
...
Harmony also has limited support for \emph{constant folding}.  For example,
...
$S_1, S_2, ...$.
which must be a set or a dictionary.  $y$ is evaluated only once at
...
This, however, excludes constants that start with the character \_: those
are considered \emph{private} to the module.
...
If it evaluated to \texttt{False} Harmony reports and error.  Harmony
