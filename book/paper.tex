...
The code in \autoref{fig:lockspec} is taken from Harmony's \texttt{synch}
module.  \autoref{fig:cssynch} shows how locks may be used.  Note that
the code of \autoref{fig:lockspec} is executable in Harmony.
...
use the keyword.  In fact, if we want to make the entire program
executable, we have to also show how \texttt{acquire} and \texttt{release}
...
\end{code}
\caption{\harmonylink{code/taslock.hny} Implementation of a spinlock
using test-and-set.}
\label{fig:tas2}
\end{figure}
...
use \textbf{atomically} by all means.
