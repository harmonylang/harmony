...
on a single value over a network.  This often has to be done in adverse circumstances,
...
Each processor \emph{proposes} a value, which we assume here is from the
set \{ 0, 1 \}.
By the definition of consensus, we want the following two propertiew:
\begin{enumerate}
\item Validity: a processor can only decide a value that has been proposed;
\item Agreement: if two processors decide, they decide the same value.
\end{enumerate}
...
\autoref{fig:bosco} presents our algorithm.  Besides the network variable, we will use a
...
The $N$ processors after go through a sequence of \emph{rounds} in which they wait for
$\texttt{N} - \texttt{F}$ messages, update their state based on the messages, and
broadcast a new messages containing their new state.
...
on that messages arrive within a particular time.
...
of its current round number and proposal.  Therefore, messages consist of a tuple containing
...
The important properties of a \textbf{select} statement are as follows:
\begin{itemize}
...
then chooses an element that is bound to the \textit{quorum} variable.
...
\item if the quorum was unanimous, decide the value.
\end{itemize}
...
\harmonysource{paxos}
\end{code}
\caption{\harmonylink{code/paxos.hny} A version of the Paxos protocol.}
\label{fig:paxos}
\end{figure}
...
\hline
\texttt{for $a$ in $e$: S} & $a$ is a bounded variable, $e$ is a set or dictionary,
...
In case of dictionaries, you can access both keys and values:
\begin{code}
