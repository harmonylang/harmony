...
The version we describe here uses \emph{public key cryptography}~\cite{pkc}: with public key
...
The original protocol goes something like this.  Suppose Alice wants to communicate
with Bob.  The three steps are as follows:
...
trying to communicate at this time.  Since it created $N_B$ recently and sent it
...
$\{ \mathtt{.dst}: p, \mathtt{.contents}: m \}$.

Unfortunately, the protocol turns out to be incorrect, but it took 17 years before
somebody noticed~\cite{Lowe95}.  The bug is easily found using model
checking~\cite{Lowe96}.
