...
\newglossaryentry{actor model}
{
  ...
  description={is a concurrency model where there are no shared variables, only
      processes with private variables and exchanging messages}
}
...
\begin{figure}
\begin{center}
\includegraphics[width=4in]{figures/mutex-crop.pdf}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=2in]{figures/rdwr-crop.pdf}
\end{center}
...
\item Figure~\ref{fig:qsort} shows an iterative implementation of the Qsort
...
in pipelines, where each upstream process is a producer and each downstream
...
\item Implement a thread-safe GPU allocator by modifying Figure~\ref{fig:gpu}.
...
\section*{Exercises}
\begin{enumerate}
\item Assume that you have only binary semaphores.  Implement counting
semaphores using binary semaphores using split binary semaphores.
\item Implement a solution to the producer/consumer problem
(Figure~\ref{fig:boundedbuffer}) using split binary semaphores.
\item Cornell's campus features some one-lane bridges.  Cars can
only go in one direction at a time. Consider northbound
...
In this chapter, we will present two SBS-based reader/writer lock
implementations that solves this type of starvation.
SBS allow much control over which thread runs next and is therefore a
good starting point for developing synchronization algorithms that do not
suffer from starvation.
For the first solution the probability that either a reader or writer gets starved
...
\section*{Exercises}
\begin{enumerate}
\item Write a fair solution to the one-lane bridge problem of Chapter~\ref{ch:sbs}.
\item TBD.  multiple GPU allocator
\end{enumerate}
...
processes that have only private memory and communicate through message queues.
...
Figure~\ref{fig:actor} shows a CXL solution to the producer/consumer problem using
an actor (as illustrated in Figure~\ref{fig:actorpic}.
