...
\begin{center}
Robbert van Renesse ~~~ William Ma ~~~ Haobin Ni ~~~ Kevin Sun ~~~ Anthony Yang
\end{center}
\end{minipage}}
...
<{:var:p}>
The Harmony <{save(e)}> expression returns a tuple containing <{e}>
and the current context.
Another thread can instantiate a context <{c}>
using the statement <{go c r}>, with the context
returning <{r}> from the <{save(e)}> expression.
This is similar to the C \texttt{setjmp/longjmp} construct.
Similar to <{save}>, the <{stop p}> expression saves the current
context in <{!p}>, where <{p}> is the address value of a shared variable.
Unlike <{save}>, the context is removed from the current set of
threads.
\autoref{fig:continuation} shows another implementation
...
that order.  Besides synchronization, scheduling, and exception handling,
continuations also enable the implementation of a primitive such as Posix
\texttt{fork} or the implementation of co-routines.
