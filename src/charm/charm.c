...
struct failure {
    enum { FAIL_SAFETY, FAIL_INVARIANT, FAIL_TERMINATION, FAIL_BUSYWAIT } type;
...
enum busywait is_stuck(struct node *start, struct node *node, uint64_t ctx, bool change) {
    ...
    if (node == start) {
        return change ? BW_RETURN : BW_ESCAPE;
    }
	if (node->visited) {
		return BW_VISITED;
	}
...
void detect_busywait(struct node *node){
    ...
    for (struct edge *edge = node->fwd; edge != NULL; edge = edge->next) {
		if (edge->node != node) {
			if (is_stuck(node, edge->node, edge->after, false) == BW_RETURN) {
                struct failure *f = new_alloc(struct failure);
...
int main(int argc, char **argv){
    ...
    *p = node;
    ...
    case FAIL_BUSYWAIT:
        printf("Busy waiting\n");
        fprintf(out, "  \"issue\": \"Busy waiting\",\n");
		break;
    default:
